...
def extract_scale(pdf_path, page_number, project_id, plan_id):
    """Whole-page (no boxes) scale presence: canonical scale string or None.

    Reads the ENTIRE page's text layer (no clip, no bounding boxes) and runs
    best_scale. Used pre-grounding as a YES/NO scale-presence signal before any
    bounding boxes exist. Mirrors is_vector's open/try/finally/close shape and,
    like it, never raises (degrades to None on any failure).
    """
    context = ctx(project_id, plan_id, page_number)
    with timed("[VECTOR_SCALE]", context, "whole-page scale presence"):
        try:
            doc = fitz.open(pdf_path)
        except Exception as e:
            logger.warning(f"[VECTOR_SCALE] [{context}] open failed: {e}; "
                           f"whole_page_scale=None")
            return None
        try:
            text = doc.load_page(page_number).get_text("text") or ""
            scale = best_scale(text)
            logger.info(f"[VECTOR_SCALE] [{context}] whole-page scale={scale}")
            return scale
        except Exception as e:
            logger.warning(f"[VECTOR_SCALE] [{context}] whole-page read failed: {e}; "
                           f"whole_page_scale=None")
            return None
        finally:
            doc.close()
