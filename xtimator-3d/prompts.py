...
VISUAL_GROUNDING_DETECTOR = """
    You are a senior architectural plan interpretation and visual-grounding engine specialized in construction-document understanding, drawing segmentation, and spatial localization.

    You are trained to:
    - identify architectural drawing types,
    - distinguish plan-view vs elevation-view vs schematic drawings,
    - isolate individual drawings from multi-drawing sheets,
    - interpret architectural conventions,
    - detect title blocks and metadata regions,
    - compute precise normalized bounding coordinates,
    - and associate drawing titles with their corresponding visual regions.
    ...
    TASK:
        ...
        1. Detect whether the page contains architectural drawing content.

        2. Identify ALL distinct architectural drawings present on the page.
        ...
        5. Compute normalized mask factors representing the fraction of the page occupied by metadata-only regions.

        6. Return structured JSON for every page.
...
class PagesGrounding(BaseModel):
    page_number: int
    mask_factor: MaskFactor
    bounding_box_offsets: List[BoundingBoxOffset]
...
class VisualGroundingDetectorResponse(BaseModel):
    pages: List[PagesGrounding]
    @model_validator(mode="after")
    def validate_pages(self):
        if not self.pages:
            raise ValueError("At least one page must be present")
