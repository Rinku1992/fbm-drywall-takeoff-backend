...
async def insert_model_2d(
    ...
    pg_pool,
    credentials,
    model_2d=dict(walls_2d=list(), polygons=list(), metadata=dict()),
    layout_2d=dict(walls_2d=list(), polygons=list(), metadata=dict()),
    insert_model=False,
    insert_layout=False,
    ):
...
async def trigger_email_notification(
    ...
    query = f"""
        SELECT plan_name FROM {credentials["CloudSQL"]["table_name_plans"]} WHERE LOWER(project_id) = LOWER(%s) AND LOWER(plan_id) = LOWER(%s)
    """
