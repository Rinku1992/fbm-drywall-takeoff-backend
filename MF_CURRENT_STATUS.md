...
Branch `feat/mf-resolver-service` off `origin/main` @ `87e6696e`, 3 commits, **not pushed, not deployed**. Everything buildable is built; nothing has been run. Full evidence in **`MF_REBUILD_REPORT.md`** (lift provenance per item, bug-fix before/after, findings).
